...
void main(void){
  ...
  for (addr = 0; addr < 128; addr++) {
    ...
    if (i2c_status == SUCCESS) {
      sprintf(string, "SUCCESS Addr: %3d    0x%2x \n", addr, addr);
      write_usb_serial_blocking(string, strlen(string));
      i++;
    }
  }

  sprintf(string, "%i devices connected to i2c bus\n", i);
  write_usb_serial_blocking(string, strlen(string));
  
  write_usb_serial_blocking("End Scanning\n", 14);
...
//  lcd_buff[lcd_buff_index++] = 0b10110001; // Set DDRAM address to 41
  ...
  lcd_key_lut['A'] = 0b11000001;
  lcd_key_lut['B'] = 0b11000010;
  lcd_key_lut['C'] = 0b11000011;
  lcd_key_lut['D'] = 0b11000100;
  lcd_key_lut['*'] = 0b10101010;
  lcd_key_lut['#'] = 0b10100011;
  ...
  while (1) {
    ...
    if (key != 0) {
      lcd_buff[lcd_buff_index++] = 0x40; // COntrol byte
