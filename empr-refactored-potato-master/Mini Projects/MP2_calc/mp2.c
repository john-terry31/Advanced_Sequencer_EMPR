...
void main(void){
  ...
  //char output[20]; // 
  int num[2]; // The two operands
  num[0] = 0;
  num[1] = 0;
  int num_index = 0; // Select if first or second operand
  ...
  while (1) {
    ...
    if (key != 0) {
      ...
      if( (lcd_key_lut[key] >= 0b10110000) && (lcd_key_lut[key] <= (0b10110000 + 10)) ) {
	write_usb_serial_blocking("Key\n", 4);
	sprintf(string, "%c", key);
	
	num[num_index] *= 10;
	num[num_index] += atoi(string);
	
	sprintf(string, "Key is %i\n", atoi(string));
	write_usb_serial_blocking(string, strlen(string));
      }
	...
	sprintf(string, "%i", res);
	for(index =0; index < strlen(string); index++)
	  lcd_buff[lcd_buff_index++] = lcd_key_lut[string[index]];
